...
export const INITIAL_LOWER_THIRD: LowerThirdState = {
    ...
    textColor: "#ffffff",
    slots: [
        { id: 1, enabled: true, type: 'logo', logoPath: '', text: 'Facebook', label: 'Facebook' },
        { id: 2, enabled: true, type: 'logo', logoPath: '', text: 'YouTube', label: 'YouTube' },
        { id: 3, enabled: true, type: 'logo', logoPath: '', text: 'TikTok', label: 'TikTok' }
    ]
};
...
class LowerThirdManager {
    ...
    private loadFromDisk(): LowerThirdState {
        try {
            ...
            if (file.size > 0) {
                const data = JSON.parse(require('fs').readFileSync(LOWER_THIRD_FILE, 'utf-8'));
